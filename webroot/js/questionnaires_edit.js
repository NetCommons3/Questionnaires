    ...
    function($scope, NetCommonsBase, NetCommonsWysiwyg,
        ...
        $scope.initialize = function(frameId, questionnaires) {
            ...
            if(Date.parse($scope.questionnaires.Questionnaire.start_period)) {
                $scope.questionnaires.Questionnaire.start_period = new Date($scope.questionnaires.Questionnaire.start_period);
            } else {
                $scope.questionnaires.Questionnaire.start_period = new Date();
            }
            if(Date.parse($scope.questionnaires.Questionnaire.end_period)) {
                $scope.questionnaires.Questionnaire.end_period = new Date($scope.questionnaires.Questionnaire.end_period);
            } else {
                $scope.questionnaires.Questionnaire.end_period = new Date();
            }
            if(Date.parse($scope.questionnaires.Questionnaire.total_show_start_period)) {
                $scope.questionnaires.Questionnaire.total_show_start_period = new Date($scope.questionnaires.Questionnaire.total_show_start_period);
            } else {
                $scope.questionnaires.Questionnaire.total_show_start_period = new Date();
            }
