...
NetCommonsApp.controller('Questionnaires.edit.question',
    function($scope, NetCommonsBase, NetCommonsWysiwyg,
        ...
        $scope.colorPickerPalette = ['#f38631', '#e0e4cd', '#69d2e7', '#68e2a7', '#f64649', '#4d5361', '#47bfbd', '#7c4f6c', '#23313c', '#9c9b7f', '#be5945', '#cccccc'];
         ...
         * @return {void}
         */
        $scope.initialize = function(frameId, questionnaire, errors, newPageLabel, newQuestionLabel, newChoiceLabel, newChoiceColumnLabel, newChoiceOtherLabel) {
            ...
            for(var pIdx=0 ; pIdx<$scope.questionnaire.QuestionnairePage.length ; pIdx++) {
                ...
                if (!$scope.questionnaire.QuestionnairePage[pIdx].QuestionnaireQuestion) {
                    continue;
                }
                for(var qIdx=0 ; qIdx<$scope.questionnaire.QuestionnairePage[pIdx].QuestionnaireQuestion.length ; qIdx++) {
                    ...
                    if($scope.questionnaire.QuestionnairePage[pIdx].QuestionnaireQuestion[qIdx].question_type == variables.TYPE_TEXT ||
                      $scope.questionnaire.QuestionnairePage[pIdx].QuestionnaireQuestion[qIdx].question_type == variables.TYPE_TEXT_AREA ||
                      $scope.questionnaire.QuestionnairePage[pIdx].QuestionnaireQuestion[qIdx].question_type == variables.TYPE_DATE_AND_TIME) {
                        $scope.questionnaire.QuestionnairePage[pIdx].QuestionnaireQuestion[qIdx].is_result_display = variables.EXPRESSION_NOT_SHOW;
                    }
                    // 日付け型の設定画面でカレンダーの開閉オプション
                    $scope.questionnaire.QuestionnairePage[pIdx].QuestionnaireQuestion[qIdx].calendar_opened = [false, false];

                    console.log($scope.questionnaire.QuestionnairePage[pIdx].QuestionnaireQuestion[qIdx].QuestionnaireChoice);
        ...
        $scope.addPage = function($event) {
            var page = new Object();
            page['page_title'] = $scope.newPageLabel + ($scope.questionnaire.QuestionnairePage.length + 1);
            ...
            $scope.addQuestion($event, $scope.questionnaire.QuestionnairePage.length-1);

            $scope.questionnaire.QuestionnairePage[$scope.questionnaire.QuestionnairePage.length-1].tab_active = true;
            if ($event) {
        ...
        $scope.deleteQuestion = function($event, pageIndex, idx, message) {
            if ($scope.questionnaire.QuestionnairePage[pageIndex].QuestionnaireQuestion.length < 2) {
                return;
            }
            if (confirm(message)) {
        ...
        $scope.addQuestion = function($event, pageIndex) {
            ...
            question['result_display_type'] = variables.RESULT_DISPLAY_TYPE_BAR_CHART;
            ...
            $scope.addChoice($event,
              pageIndex,
              $scope.questionnaire.QuestionnairePage[pageIndex].QuestionnaireQuestion.length - 1,
              0,
         ...
         * @return {void}
         */
        $scope.moveQuestionToAnotherPage = function($event, pageIndex, qIndex, movePageIndex) {
            var tmp_q = $scope.questionnaire.QuestionnairePage[pageIndex].QuestionnaireQuestion.splice(qIndex, 1);
            $scope.questionnaire.QuestionnairePage[movePageIndex].QuestionnaireQuestion.push(tmp_q[0]);
         ...
         * @return {void}
         */
        $scope.addChoice = function($event, pIdx, qIdx, choiceCount, otherType, matrixType) {
            ...
            if (otherType != variables.OTHER_CHOICE_TYPE_NO_OTHER_FILED) {
                ...
                if (matrixType == variables.MATRIX_TYPE_ROW_OR_NO_MATRIX) {
                    choice['choice_label'] = $scope.newChoiceLabel + (choiceCount + 1);
                } else {
                    choice['choice_label'] = $scope.newChoiceColumnLabel + (choiceCount + 1);
                }
            }
            ...
            choice['origin_id'] = 0;
            if (otherType != variables.OTHER_CHOICE_TYPE_NO_OTHER_FILED) {
                choice['graph_color'] = $scope.colorPickerPalette[choice['choice_sequence'] % 12];
            } else {
                choice['graph_color'] = $scope.colorPickerPalette[choiceCount % 12];
            }
        ...
        $scope.deleteChoice = function($event, pIdx, qIdx, seq) {
            if ($scope.questionnaire.QuestionnairePage[pIdx].QuestionnaireQuestion[qIdx].QuestionnaireChoice.length < 2) {
                return;
            }
            for(var i=0; i<$scope.questionnaire.QuestionnairePage[pIdx].QuestionnaireQuestion[qIdx].QuestionnaireChoice.length; i++) {
